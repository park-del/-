...
char p[70];  char s1[70],s2[70]; int t1=0,t2=0;
char A1,A2;
/*********************************************************************************************************
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////
void zhuanhuan(char s1[],int t1,char s2[],int t2)
{
...
void sendp_s1_s2(char *p)   //p里面存放的是2513.3072,N,10346.3723,E
{
  int i=0;  t1=0; t2=0;
  while(p[i]!=',')
  {
...
{   
   ...
   while(1)
   {
	 ...
	 myDelay(1);
	 sendm_p(m);     //取出那一串字符放到p中
	 myDelay(1);
	 sendp_s1_s2(p);  //将p中的那一串字符的数字部分分开放到s1和s2中
	 myDelay(1);
	 zhuanhuan(s1,t1,s2,t2);
	 myDelay(1);  
   }		
   return 0;
}		