...
void OSInit (OS_ERR *p_err)
{
	...
	OSRunning =  OS_STATE_OS_STOPPED;   //配置OS初始状态为停止态 
	
	
	OSTCBCurPtr = (OS_TCB *)0;          //初始化两个全局TCB，这两个TCB用于任务切换
	OSTCBHighRdyPtr = (OS_TCB *)0;
	
	
	OSPrioCur                       = (OS_PRIO)0;   //初始化优先级变量 
  OSPrioHighRdy                   = (OS_PRIO)0;
	
	
	OS_PrioInit();     //初始化优先级表(即是对优先级表中的所有位都置为1)
	
	OS_RdyListInit();  //初始化就绪列表(就绪列表中每一个元素都是有3个变量的，这3个变量分别为number，head，tail，这里是为就绪列表中每一个元素的这3个变量都置为0)
	
	
	OS_IdleTaskInit(p_err);  //初始化空闲任务(在执行空闲任务的初始化函数的时候就完成了对空闲任务的创建此时在优先级表中的最后一位是置为1了的)
...
void OSStart (OS_ERR *p_err)    //启动RTOS，将不再返回
{	
	if( OSRunning == OS_STATE_OS_STOPPED )
	{
		...
		OSTCBHighRdyPtr = OSRdyList[OSPrioHighRdy].HeadPtr;		//找到最高优先级的TCB（OSPrioHighRdy指针指向的那一个任务就是在进行任务切换时所切换到的那一个任务）
...
void OSSched(void)  //任务切换，实际就是触发PendSV异常，然后在PendSV异常中进行上下文切换
{
	...
	OS_CRITICAL_ENTER();   //进入临界区（在临界区中执行的语句是不能够被中断所打断的）
	
	 
	OSPrioHighRdy   = OS_PrioGetHighest();   //查找最高优先级的任务
	OSTCBHighRdyPtr = OSRdyList[OSPrioHighRdy].HeadPtr;	
	
	
	if (OSTCBHighRdyPtr == OSTCBCurPtr)      //如果最高优先级的任务是当前任务则直接返回，不进行任务切换
	{
...
void  OS_IdleTaskInit(OS_ERR  *p_err) //空闲任务初始化
{	
 
	OSIdleTaskCtr = (OS_IDLE_CTR)0;
	
	 
	OSTaskCreate( (OS_TCB     *)&OSIdleTaskTCB,  //创建空闲任务
