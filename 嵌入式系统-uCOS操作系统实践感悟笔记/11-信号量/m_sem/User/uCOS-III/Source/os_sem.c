                   ...
                   OS_ERR      *p_err)  //返回错误类型
{
	...
	/*
	在信号量的数据结构“struct  os_sem”是有3个主要的成员的，一个是 p_sem->NamePtr（用来表示p_sem这个信号量的名字），一个是 p_sem->Ctr（用来表示这个信号量的初值），一个是
	p_sem->PendList（用来表示这个信号量的等待列表）。其中	p_sem->PendList 中有一个HeadPtr成员，指向了这个信号量的等待列表的第1个结点 ，TailPtr指向了这个信号量的等待列表中的
	最后一个结点，NbrEntries表示了这个信号量等待列表中的结点数量（即是等待这个信号量的任务的数量的）。
	
	创建信号量时 p_sem->NamePtr = p_name用来设置这个信号量的名称，p_sem->Ctr =cnt用来设置这个信号量的初值，即这个信号量的初始资源数，OS_PendListInit(&p_sem->PendList)是用来
	初始话这个信号量p_sem对应的等待列表的。
	*/
                       ...
                       OS_ERR   *p_err)   //返回错误类型
{
    ...
    if (p_ts != (CPU_TS *)0) {                //如果 p_ts 非空
       *p_ts  = (CPU_TS)0;                    //初始化（清零）p_ts，待用于返回时间戳
		...
		//下面就是关键部分
    CPU_CRITICAL_ENTER();                     //关中断
    if (p_sem->Ctr > (OS_SEM_CTR)0) {         //如果资源可用的话肯定不会是阻塞的，直接返回就可以了
        p_sem->Ctr--;                         //资源数目减1
			
        if (p_ts != (CPU_TS *)0) {            //如果 p_ts 非空
           *p_ts  = p_sem->TS;                //获取该信号量最后一次发布的时间戳，用*p_ts记录
        }
       ...
       *p_err = OS_ERR_NONE;                  //返回错误类型为“无错误”
		...
		//不满足上面那个条件说民了p_sem->Ctr为<=0,即当前没有资源
    ...
    //上面都不满足说明了没有资源可用，但选择了阻塞任务，此时调度器并没有被上锁，是可以进行任务切换的（如果此时调度器被上了锁的话又选择了没有申请到信号量时阻塞该任务，则此时就会
		//出现没有任务执行的情况，系统就会出现问题），此时执行的就是将这个没有申请到信号量所对应的资源的当前任务进行阻塞，从就绪列表中插入到时基列表中，同时设置了其在时基列表中睡眠
    //的时间为timeout，如果时间到了的时候，在时钟中断中会将其重新从时基列表中插入到就绪列表中的。同时也将这个任务插入到了等待该信号量的列表中了，在一个任务释放信号量的时候是可能
    //会将其唤醒的		
    OS_CRITICAL_ENTER_CPU_EXIT();                           //锁调度器，并重开中断
		...
		/*
			上面的OS_Pend()函数所执行的功能就是将当前这个没有申请到信号量对应资源任务从就绪列表中删除，同时
    */						
						
    OS_CRITICAL_EXIT_NO_SCHED();                            //开调度器，但不进行调度

    OSSched();                                              //找到并调度最高优先级就绪任务（执行到这里的时候说明了进行任务切换就会切换到另外的一个任务，当前任务位于时基列表
		//中处于阻塞的状态）
   
						
		/* 当前任务（获得信号量）得以继续运行，执行到这里说明了当前任务从OSSched()处回来了，即是当前任务右重新获得了信号量或者设置的阻塞时间timeout到了的时候自动苏醒过来  */
    CPU_CRITICAL_ENTER();                                   //关中断
...
//发送信号量的函数，当按键检测到按键按下时就会执行这个函数发送一个信号量，发送之前首先是要调用OSSemCreate()函数来创建一个信号量的
                       ...
                       OS_ERR  *p_err)    //返回错误类型
{
    ...
    ts = OS_TS_GET();                             //获取时间戳（调用这个函数可以获取这个信号量的时间戳，信号量的时间戳就是信号量最后释放的时间的，即是最后一次释放对应的时钟周期数）
    ...
    ctr = OS_SemPost(p_sem,                       //将信号量按照普通方式处理（如果 OSSemPost()这个函数没有在中断中被调用的话）
                     opt,
                     ts,
                     p_err);
...
//发送一个信号量（发送信号量就是释放信号量，信号的资源数要加1）的时候肯定是要判断是否有任务在等待当前这个信号量的，如果有的话则是要从这个信号量的等待列表中唤醒一个任务的
...
{
    OS_OBJ_QTY     cnt;
    OS_SEM_CTR     ctr;    //从这里就可以看到OS_SEM_CTR就是为ctr变量的类型的
    OS_PEND_LIST  *p_pend_list;
    OS_PEND_DATA  *p_pend_data;
    ...
    if (p_pend_list->NbrEntries == (OS_OBJ_QTY)0) {         //如果没有任务在等待该信号量
        ...
        switch (sizeof(OS_SEM_CTR)) {                       //判断是否将导致该信号量计数值溢出，
            case 1u:                                        //如果溢出，则开中断，返回错误类型为
                 if (p_sem->Ctr == DEF_INT_08U_MAX_VAL) {   //“计数值溢出”，返回0（有错误），    即是OS_SEM_CTR如果为1u的话（OS_SEM_CTR表示信号量的Ctr变量的类型），就是为1个无符号数，此时如果p_sem->Ctr值为255的话在，则其再进行加1的话就会发生溢出
       ...
       *p_err     = OS_ERR_NONE;                            //返回错误类型为“无错误”
        return (ctr);                                       //返回信号量的计数值，不继续执行（此时直接返回就行了，因为等待队列中没有任何的任务需要被唤醒）
    }

		//执行下面的说明了该信号量的等待队列中有正在等待的任务需要被唤醒
    OS_CRITICAL_ENTER_CPU_EXIT();                           //加锁调度器，但开中断
		
    if ((opt & OS_OPT_POST_ALL) != (OS_OPT)0) {             //如果要将信号量发布给所有等待任务（由于是可以选择释放信号量时不进行任务切换的，故在执行信号量对应的资源数是可能有多个的，是可以同时唤醒多个任务的，这里选择的是唤醒1个任务还是唤醒多个任务）
        cnt = p_pend_list->NbrEntries;                      //获取等待任务数目到 cnt
    } else {                                                //如果要将信号量发布给优先级最高的等待任务
        cnt = (OS_OBJ_QTY)1;                                //将要操作的任务数为1，cnt 置1
    }
		
    p_pend_data = p_pend_list->HeadPtr;                     //获取等待列表的首个任务到 p_pend_data（即是等待队列中的第1个结点）
		
    while (cnt > 0u) {                                      //逐个处理要发布的任务
        p_tcb            = p_pend_data->TCBPtr;             //取出当前任务
        p_pend_data_next = p_pend_data->NextPtr;            //取出下一个任务
        
			OS_Post((OS_PEND_OBJ *)((void *)p_sem),               //发布信号量给当前任务，即是唤醒当前任务（在这个函数里面进行的只是将相应的任务从一个表中删除，插入到另外的一个表，是并没有进行任务切换的）
                p_tcb,
                (void      *)0,
                (OS_MSG_SIZE)0,
                ts);
		...
		//将信号量发布给所有的等待任务时是没有进行任务切换的，而将信号量只发布给一个任务时是有进行任务切换的
		
    ctr = p_sem->Ctr;                                       //获取信号量计数值到 ctr
    OS_CRITICAL_EXIT_NO_SCHED();                            //减锁调度器，但不执行任务调度
    
		if ((opt & OS_OPT_POST_NO_SCHED) == (OS_OPT)0) {        //如果 opt 没选择“发布时不调度任务”（这里选择的是释放信号量时不进行任务切换）
        OSSched();                                          //任务调度
    }
   *p_err = OS_ERR_NONE;                                    //返回错误类型为“无错误”
