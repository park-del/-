...
static  void  AppTaskKey ( void * p_arg )
{
	...
	while (DEF_TRUE) {                                                         //任务体
		...
		/*
	  由于AppTaskKey任务的优先级比AppTaskLed1任务的优先级高，故系统会先执行AppTaskKey。
		
		系统执行这个AppTaskKey任务的，就是一直循环执行while里面的“if（按键检测）”和“	OSTimeDlyHMSM()”，注意，这里是加上了一个OSTimeDlyHMSM()函数的，因为任务
		AppTaskKey的优先级是比任务AppTaskLed1任务的优先级高的，加上这个可以保证这两个任务都能够在运行，即相当于是同时运行。
		
		当本按键检测的任务执行到“if(按键检测)”时发现条件为真，即是有某一个按键被按下了，则本“按键检测任务”就会执行OSSemPost()函数发送一个SemOfKey信号。
...
static  void  AppTaskLed1 ( void * p_arg )
{
    ...
    while (DEF_TRUE) {                              //任务体
			...
			/*
			该AppTaskLed1任务会一直循环执行while循环语句里面的内容，当该任务执行到OSSemPend()函数的时候会申请一个信号量。
